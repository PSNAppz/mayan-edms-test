...
from .permissions import (
    permission_theme_delete,
    permission_theme_edit,
...
class AppearanceApp(MayanAppConfig):
    ...
    def ready(self):
        ...
        ModelCopy(bind_link=True, model=Theme, register_permission=True).add_fields(
            field_names=(
                "label",
                "stylesheet",
            )
        ...
        ModelEventType.register(event_types=(event_theme_edited,), model=Theme)
        ModelEventType.register(
            event_types=(event_user_theme_settings_edited,), model=User
        )
        ...
        SourceColumn(
            attribute="label", is_identifier=True, is_sortable=True, source=Theme
        )
        ...
        menu_object.bind_links(
            links=(link_theme_delete, link_theme_edit), sources=(Theme,)
        )
        menu_return.bind_links(
            links=(link_theme_list,),
            sources=(Theme, "appearance:theme_list", "appearance:theme_create"),
        )
        ...
        menu_object.bind_links(links=(link_user_theme_settings_edit,), sources=(User,))
