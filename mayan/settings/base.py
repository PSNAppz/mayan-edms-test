...
MEDIA_ROOT = os.environ.get("MAYAN_MEDIA_ROOT", Path(BASE_DIR, "media"))
from pathlib import Path
import os
...
if COMMAND_NAME_SETTINGS_REVERT in sys.argv:
    ...
    DATABASES = {
        "default": {
            ...
            "NAME": Path(BASE_DIR, "db.sqlite3"),  # NOQA: F821
        }
    }
else:
...
try:
    ...
    path_secret_key = Path(MEDIA_ROOT, SYSTEM_DIR, SECRET_KEY_FILENAME)  # NOQA: F821
    try:
...
PAGINATION_SETTINGS = {"PAGE_RANGE_DISPLAYED": 5, "MARGIN_PAGES_DISPLAYED": 2}
...
COMMON_EXTRA_APPS_PRE = []  # Define COMMON_EXTRA_APPS_PRE with appropriate value

COMMON_EXTRA_APPS = []  # Define COMMON_EXTRA_APPS with appropriate value

COMMON_DISABLED_APPS = []  # Define COMMON_DISABLED_APPS with appropriate value

repeated_apps = tuple(set(COMMON_EXTRA_APPS_PRE).intersection(set(COMMON_EXTRA_APPS)))
if repeated_apps:
    raise ImproperlyConfigured(
        'Apps "{}" cannot be specified in `COMMON_EXTRA_APPS_PRE` and '
        "`COMMON_EXTRA_APPS` at the same time.".format(", ".join(tuple(repeated_apps)))
    )
...
INSTALLED_APPS = [
    APP
    for APP in INSTALLED_APPS
    if APP not in (COMMON_DISABLED_APPS or ())  # NOQA: F821
]

DATABASE_ENGINE = os.environ.get("MAYAN_DATABASE_ENGINE")
DATABASE_NAME = os.environ.get("MAYAN_DATABASE_NAME")
DATABASE_USER = os.environ.get("MAYAN_DATABASE_USER")
DATABASE_PASSWORD = os.environ.get("MAYAN_DATABASE_PASSWORD")
DATABASE_HOST = os.environ.get("MAYAN_DATABASE_HOST")
DATABASE_PORT = os.environ.get("MAYAN_DATABASE_PORT")
DATABASE_CONN_MAX_AGE = os.environ.get("MAYAN_DATABASE_CONN_MAX_AGE", 0)
